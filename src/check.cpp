...
void testFps(int fps, image_transport::Publisher &pub, ZEDCamera &cam)
{
...
void testResolution(int res, int fps, image_transport::Publisher &pub, ZEDCamera &cam)
{
...
int main(int argc, char **argv)
{
    ...
    for (int id = 0; id < 10; id++) {
        ...
        if (cam.ok()) {
            ...
            if (res >= 0)
                while(ros::ok()) testResolution(res, fps, pub, cam);
            else for (res = 3; res >= 0; --res)
                testResolution(res, fps, pub, cam);
            return EXIT_SUCCESS;
        }
    }
